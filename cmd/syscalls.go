...
func init() {
	lib.Commands["syscalls"] = syscalls
	lib.Args["syscalls"] = syscallsArgs{}
}

type syscallsArgs struct {
	ContainerID string `arg:"positional,required"`
	Start       bool   `arg:"-s,--start" help:"call docker-start on ContainerID"`
}

func (syscallsArgs) Description() string {
	return "\nbpftrace syscalls in a running container\n"
}

const syscallsDockerfile = `
...
const syscallsBpftrace = `#!/usr/bin/env bpftrace
...
{
    printf("%d\t%d\t%s\t%d\n", pid, curtask->real_parent->pid, comm, args->id);
}

`

func syscalls() {
	var args syscallsArgs
	arg.MustParse(&args)
	//
	syscalls, err := ioutil.ReadDir("/sys/fs/cgroup/")
	if err != nil {
		lib.Logger.Fatal("error: ", err)
	}
	fail := true
	for _, file := range syscalls {
	...
	if err != nil {
		...
		_, err = w.Write([]byte(syscallsDockerfile))
		if err != nil {
			lib.Logger.Fatal("error: ", err)
		}
		err = w.Close()
	...
	err = ioutil.WriteFile(tempDir+"/syscalls.bt", []byte(strings.ReplaceAll(syscallsBpftrace, "CONTAINERID", args.ContainerID)), 0666)
	if err != nil {
		lib.Logger.Fatal("error: ", err)
	}
	...
	//
	// to see all syscalls, bpftrace needs to start before the container, but in that case the cgroup directory doesn't exist yet and must be created
	...
	out, err := cli.ContainerCreate(
		ctx,
		&container.Config{
			Cmd:   []string{"bpftrace", "/bpftrace/syscalls.bt"},
			Image: "docker-trace:bpftrace",
	...
	if !(strings.HasPrefix(string(line[8:]), "Attaching ") && strings.HasSuffix(string(line[8:]), " probe...\n")) {
	...
	go func() {
		for {
			...
			case 1:
				...
				ppid := parts[1]
				comm := parts[2]
				syscall := lib.Atoi(parts[3])
				fmt.Println(pid, ppid, comm, syscallNames[syscall])
			case 2:
